...
// region Device & App Info
public struct YQSUIDevice {
    ...
    public static var modelName: String {
        if isSimulator {
            let simulatorIdentifier = ProcessInfo.processInfo.environment["SIMULATOR_MODEL_IDENTIFIER"] ?? model
            let mapped = modelNameMapping[simulatorIdentifier] ?? simulatorIdentifier
            return "Simulator (\(mapped))"
        }
        return modelNameMapping[model] ?? model
    }
